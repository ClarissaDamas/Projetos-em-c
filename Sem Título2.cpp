...
void ordenaPorFrequencia(stLst *lst) {
    int trocou;
    stElem *atual, *prev = NULL, *fim = NULL; //inicia com ponteiro zerado

    do {
        trocou = 0; //variavel de troca
        atual = lst->first; //atribui first na lista

        while (atual->next != fim) { //enquanto nao for fim
            if (atual->f < atual->next->f) {  // enquanto o atual for menor que a frequencia
