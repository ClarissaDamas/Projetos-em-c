...
typedef struct Elem {
    char c[99];      // Caracter (string de até 99 caracteres)
    int f;           // Frequência de caractere
    int i;           // Posição do elemento na lista
...
void concatenaCaracteres(stElem *novo, stElem *filhoE, stElem *filhoD) {
    ...
    while (j < 99 && filhoD->c[j] != '\0') {
        novo->c[i] = filhoD->c[j];
        i++; j++;  // Avança no vetor de novo->c e no vetor filhoD
    }
...
stElem *geraArvoreHuff(stLst *lst) {
    ...
    while (lst->n > 1) {
        if (primeiro != NULL && segundo != NULL) {
            novo = malloc(sizeof(stElem));  // Aloca memória para o novo nó
            ...
            if (novo) {
                ...
                printf("\nProblema na alocação...Na função geraArvoreHuff\n");
                break;
            }
        }
    ...
    return lst->first;  // Retorna o nó raiz da árvore
}
...
int main(void) {
    ...
    if (!pArq) {
        printf("\nArquivo em referência com problema...\n");
        exit(1);
    }
