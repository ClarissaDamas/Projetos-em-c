...
*/
void geraPoliRegular(int n, float vx[], float vy[], float va[]) { //va = vetor de angulos
	int i;
	...
	for(i=0; i<n; i++) {
		...
		normaB = prodVetor(vx[i], vy[i], vx[i], vy[i]); //calcular a norma do vetor, ja q com a norma
		//e o arcocosseno consigo encontrar o angulo
		normaB = sqrt(normaB);
		cosAng /= normaB;
		arcos = acos(cosAng);  
		// correcao
